 ...
 * 此时通知所有其它在线的用户，刚加入的用户是谁
 */
function onConnect (tunnelId) {
    console.log(`[onConnect] =>`, { tunnelId })

    if (tunnelId in userMap && !connectedTunnelIds.includes(tunnelId)) {
 ...
 * 在本示例，我们处理 `rscore` 类型的消息，该消息表示有新的分数记录来了。
    ...
    switch (type) {
        case 'rscore':
            if (tunnelId in userMap) {
                userMap[tunnelId].score = content.score
                $broadcast('rscore', {
                    'who': userMap[tunnelId],
                    'score': content.score
                })
